...
## 同步
### 密碼頁
- 「密碼」使用獨立的 Google Drive 文件；在後台填入「密碼文件 ID」後即可開啟。
- 桌面版左側是四欄網站選擇器，右側顯示該網站的帳密；手機版點網站後以浮窗顯示。四欄在檔案內使用 A、B、C、D，顯示名稱映射也保存在同一份密碼檔案並可直接修改。
- 每組帳密固定顯示名稱、帳號、密碼、備註四行，密碼預設遮蔽。帳號與密碼可點擊複製，成功時按鈕會短暫變綠。網站網址可選填，並以新視窗開啟。
- 密碼修改會保留本機副本並自動同步；若雲端與本機同時更新，會暫停並讓你選擇版本。
