...
## 新功能
- 後台新增「iPhone 離線提醒測試」：一次設定 Apple 捷徑後，可傳送測試標題與時間至提醒事項，再手動驗證緊急提醒在飛航模式下響鈴。此階段不串接正式 Action，不會自動同步改期／取消，也不代表網頁已確認手機建立成功。完整捷徑設定步驟在後台卡片內。
...
- 刷新页面通常不用重登
- 同一浏览器会话通常不用重登
- token 到期、瀏覽器重新整理或斷線恢復後，會先嘗試自動登入並接續同步；若瀏覽器要求互動，再點「登入」
- Google 登入元件載入失敗或逾時可點登入重試；若仍在載入，看到就緒訊息後再點一次，以直接點擊開啟登入視窗。瀏覽器限制暫存不會中斷初始化，但無法記住工作階段。

目前的自動登入與「續期」仍是 Google 瀏覽器授權流程，可能顯示視窗；取消或被阻擋後不會頻繁重試，可自行點登入重新連接。重新連接會恢復定期同步，並保留尚未上傳的編輯內容。主動按「登出」後不會再自動登入。
...
## 同步
### 密碼、通訊錄與小卡
- 「密碼」、「通訊錄」與「小卡」共用一份獨立的 Google Drive 文件；在後台填入共用文件 ID 後即可開啟。
- 密碼頁桌面版左側是四欄網站選擇器，右側顯示該網站的帳密；手機版點網站後以浮窗顯示。四欄在檔案內使用 A、B、C、D，顯示名稱映射也保存在共用檔案並可直接修改。
- 每組帳密固定顯示名稱、帳號、密碼、備註四行，密碼預設遮蔽。帳號與密碼可點擊複製，成功時按鈕會短暫變綠。網站網址可選填；有網址時網站名稱本身就是連結。
- 通訊錄同樣分成四欄，每張主卡片是一家公司；右側會顯示公司網站、電話、地址，以及每位聯絡人的姓名、部門／職稱、可重複新增的電話與證件號、地址及備註。
- 小卡頁以拼貼牆呈現常用資訊。文字小卡可直接複製文字或轉成圖片複製；圖片小卡從獨立的小卡圖片資料夾載入，點擊即可複製圖片。
- 共用資料修改會保留本機副本並自動同步；若雲端與本機同時更新，會暫停並讓你選擇版本。
...
- 網路恢復時會自動恢復登入與同步；若瀏覽器阻擋自動授權，則先保留本機筆記，按登入後再同步。登出不清除筆記，並會停止自動連線，直到再次按登入。
