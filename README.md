...
## 同步
### 密碼與通訊錄
- 「密碼」與「通訊錄」共用一份獨立的 Google Drive 文件；在後台填入「密碼與通訊錄文件 ID」後即可開啟。
- 密碼頁桌面版左側是四欄網站選擇器，右側顯示該網站的帳密；手機版點網站後以浮窗顯示。四欄在檔案內使用 A、B、C、D，顯示名稱映射也保存在共用檔案並可直接修改。
- 每組帳密固定顯示名稱、帳號、密碼、備註四行，密碼預設遮蔽。帳號與密碼可點擊複製，成功時按鈕會短暫變綠。網站網址可選填，並以新視窗開啟。
- 通訊錄同樣分成四欄，每張主卡片是一家公司；右側每張小卡片是一位聯絡人，包含姓名、部門／職稱、可重複新增的電話與證件號、地址及備註。
- 密碼與通訊錄修改都會保留本機副本並自動同步；若雲端與本機同時更新，會暫停並讓你選擇版本。
