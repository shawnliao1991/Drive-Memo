...
function create({root,content,getText,mutate,renderBullet,onRange}){
 ...
 for(let i=1;i<=days;i++){const date=month+'-'+String(i).padStart(2,'0'),c=counts[date]||{},counters=content.PRIORITIES.map(p=>`<span class="calendar-count ${p}">${c[p]||0}</span>`).join('');cells+=`<button type="button" class="month-day ${date===content.localDateKey()?'is-today':''}" data-month-date="${date}" data-drop-date="${date}" aria-label="${date}"><strong>${i}</strong><span class="day-counts">${counters}</span>${c.pending?`<span class="pending-count">待確認 ${c.pending}</span>`:''}</button>`}
 calendar.innerHTML=`<div class="calendar-head"><button class="btn" data-month-step="-1" aria-label="上個月">‹</button><strong>${y} 年 ${m} 月</strong><button class="btn" data-month-step="1" aria-label="下個月">›</button></div><div class="calendar-legend">Actions（含 Done） · <span class="urgent">立即</span> <span class="today">今天</span> <span class="soon">這幾天</span> <span class="someday">慢慢做</span></div><div class="month-grid">${['日','一','二','三','四','五','六'].map(n=>`<span class="weekday">${n}</span>`).join('')}${cells}</div><div id="calendarActions"></div>`;
