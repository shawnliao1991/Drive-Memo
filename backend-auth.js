...
function create({onIdentity=()=>{},onConnected=()=>{},onReady=()=>{},onStatus=()=>{}}={}){
 let token=null,expires=0,pending=null,popup=null,generation=0,signedOut=false;
 async function getToken(force=false){
  if(signedOut)throw Error("AUTH_EXPIRED");const requestGeneration=generation;
  ...
  pending=(async()=>{let response;try{response=await fetch("/auth/token",{method:"POST",credentials:"same-origin",headers:{"X-Drive-Memo":"1",...(force?{"X-Drive-Memo-Refresh":"1"}:{})},cache:"no-store"})}catch{throw Error("暫時無法連接同步服務，請稍後再試")}
   ...
   const value=await response.json();if(requestGeneration!==generation||signedOut)throw Error("AUTH_EXPIRED");token=value.access_token;expires=Date.now()+Number(value.expires_in)*1000;onIdentity(value.email);onConnected(true);return token;
 ...
 function login(){signedOut=false;if(popup&&!popup.closed){popup.focus();return}popup=global.open("/auth/login","drive-memo-google","popup,width=520,height=680");if(!popup)onStatus("請允許登入視窗，然後再點登入","err")}
 ...
 async function logout(){signedOut=true;generation++;token=null;expires=0;popup?.close?.();popup=null;onConnected(false);onIdentity("");const response=await fetch("/auth/logout",{method:"POST",credentials:"same-origin",headers:{"X-Drive-Memo":"1"}});if(!response.ok)throw Error("登出失敗，請稍後再試")}
