...
function updateBullet(markdown,bulletId,patch={}){return updateJournal(markdown,journal=>{const bullet=journal.bullets.find(value=>value.id===bulletId);if(!bullet)return;const item=journal.items.find(value=>value.id===bullet.itemId),updatedAt=nowIso();if(item){if(patch.title!==undefined)item.title=String(patch.title||"未命名項目").slice(0,160);if(patch.details!==undefined)item.details=String(patch.details||"").slice(0,MAX_DETAILS_LENGTH);item.updatedAt=updatedAt}if(patch.action!==undefined)bullet.action=String(patch.action||"").slice(0,500);if(patch.priority!==undefined&&PRIORITIES.includes(patch.priority))bullet.priority=patch.priority;if(patch.date!==undefined&&/^\d{4}-\d{2}-\d{2}$/.test(patch.date)&&patch.date!==bullet.date){bullet.dateHistory=[...new Set([...(bullet.dateHistory||[]),bullet.date])];bullet.date=patch.date}bullet.updatedAt=updatedAt})}
