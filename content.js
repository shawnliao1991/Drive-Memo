...
function createEditSession(){const changes=new Map(),equal=(a,b)=>JSON.stringify(a)===JSON.stringify(b);return{
 track(before,after){const a=parse(before).journal,b=parse(after).journal;for(const kind of ["items","bullets"]){const previous=new Map(a[kind].map(record=>[record.id,record]));for(const record of b[kind]){const prior=previous.get(record.id);if(equal(prior,record))continue;const key=kind+":"+record.id;let change=changes.get(key);if(!change){change={kind,id:record.id,created:!prior,fields:{}};changes.set(key,change)}for(const field of Object.keys(record)){if(equal(prior?.[field],record[field]))continue;if(!change.fields[field])change.fields[field]={before:prior?.[field]};change.fields[field].after=record[field]}}}},
