...
「牛馬日誌」是以日期為主軸的個人 Bullet Journal。使用者每天記下要推進的事情，快速判斷現在該做什麼、接下來該做什麼，也能沿著日期回顧過去完成與順延的工作。
...
4. 紅色與綠色 Bullet 即使順延，仍在原日期的 Highlight 區留下紀錄；當後續串接的任務完成時，原日期 Highlight 改為灰色。未完成時保留原重要度顏色，清楚呈現當天未達成的重點。
...
- 頂部顯示今天日期、前後日期切換與新增 Bullet 按鈕。
- 「現在該做什麼」優先顯示紅色與綠色 Bullet。
- 「接下來做什麼」顯示藍色與無色 Bullet。
- 「今天完成」顯示已完成的灰色 Bullet，可預設收合。
- 預設排序為紅、綠、藍、無色；同重要度內保留手動排序，已完成項目移到當日完成區但不改變歷史順序。
- 日期時間軸參考現有圖片：按日期分組、可展開與收合，使用圓點、縮排與細線表達層級。
