...
let selectedDate=Content.localDateKey(),calendarMonth="",lastDeletedId=null,toastTimer=null,identityValue="";
...
function renderProjects(){
 ...
 els.projectsList.innerHTML=projects.length?projects.map(project=>`<article class="panel project-card ${project.projectStatus==="closed"?"closed":""}"><button class="project-main" data-action="open-project" data-id="${escapeHtml(project.bulletId||"")}" ${project.bulletId?"":"disabled"}><span class="project-title-row"><strong class="project-title">${escapeHtml(project.title)}</strong><span class="project-badge">${project.projectStatus==="closed"?"已結案":"進行中"}</span></span><span class="project-action">${escapeHtml(project.action)}</span><span class="project-meta">${project.date?`<time datetime="${project.date}">${escapeHtml(formatDateLabel(project.date))}</time>`:""}<span>${escapeHtml(priorityNames[project.priority])}</span></span></button><div class="project-actions">${project.bulletId?`<button class="btn" data-action="open-project" data-id="${escapeHtml(project.bulletId)}">編輯</button>`:""}<button class="btn" data-action="project-status" data-item-id="${escapeHtml(project.id)}" data-status="${project.projectStatus==="closed"?"open":"closed"}">${project.projectStatus==="closed"?"重新開啟":"結案"}</button></div></article>`).join(""):emptyMessage("還沒有專案。可在編輯 Bullet 時將事情轉為專案。");
}
...
function openBulletDialog(id=""){
 ...
 els.bulletDialogTitle.textContent=bullet?bullet.kind==="project"?"編輯專案 Bullet":"編輯 Bullet":"新增 Bullet";els.bulletId.value=bullet?.id||"";els.bulletTitle.value=bullet?.title||"";els.bulletAction.value=bullet?.action||"";setBulletDate(bullet?.date||selectedDate);setPriority(bullet?.priority||"someday");els.bulletDetails.value=bullet?.details||"";els.bulletDetailsLabel.textContent=bullet?.kind==="project"?"專案細節":"事情細節";els.deleteBulletBtn.classList.toggle("hidden",!bullet);els.convertProjectBtn.classList.toggle("hidden",!bullet||bullet.kind==="project");els.bulletHistoryField.classList.toggle("hidden",!bullet);els.bulletHistory.innerHTML=history.map(entry=>`<div class="history-row"><time>${escapeHtml(entry.date)}</time><span class="history-action">${escapeHtml(entry.action)}</span><span class="history-state">${entry.status==="done"?"已完成":entry.status==="migrated"?"已順延":"未完成"}</span></div>`).join("");toggleDatePicker(false);els.bulletDialog.showModal();setTimeout(()=>els.bulletTitle.focus(),0)
}
